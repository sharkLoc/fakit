...
pub fn file_reader(file_in: &Option<String>) -> Result<Box<dyn BufRead>> {
    if let Some(file_name) = file_in {
        ...
        if flag {
            ...
            Ok(
                Box::new(
                    BufReader::with_capacity(1024 * 256, fp)
                )
            )
        }
    } else {
...
#[allow(dead_code)]
pub fn file_writer(file_name: &str) -> Result<Box<dyn Write>> {
    let fp = File::create(file_name)?;
    if file_name.ends_with(".gz") || file_name.ends_with(".gzip") {
        Ok(
            Box::new(
                BufWriter::with_capacity(1024 * 256,write::GzEncoder::new(fp,Compression::default()) )
            )
        )
    } else {
        Ok(
            Box::new(
                BufWriter::with_capacity(1024 * 256, fp)
            )
        )
    }
}
